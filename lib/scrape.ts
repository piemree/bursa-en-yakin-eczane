...
function parseCoordinates(mapsHref: string | undefined): { lat: number; lng: number } | null {
...
function parseAddressBlock(html: string): { address: string; landmark: string } {
  const withoutPhones = html.replace(/<a[^>]*href="tel:[^"]*"[^>]*>.*?<\/a>/gi, "");
  const parts = cheerio.load(`<div>${withoutPhones}</div>`)("div")
    .text()
    .replace(/\s+/g, " ")
    .trim();
...
export function parseNobetciHtml(html: string): Eczane[] {
  ...
  $(".nobetci").each((index, element) => {
    ...
    const dutyText = block.find("span.red").first().text().replace(/\s+/g, " ").trim();
    const [dutyStart = "", dutyEnd = ""] = dutyText.split("/").map((part) => part.trim());
    ...
    eczaneler.push({
      ...
      dutyStart,
      dutyEnd,
      mapsUrl: mapsHref ?? `https://www.google.com/maps?q=${coords.lat},${coords.lng}`,
    });
  });
...
export async function getNobetciEczaneler(): Promise<NobetciData> {
  const response = await fetch(BEO_URL, {
    ...
    headers: {
      "User-Agent": "BursaNobetciEczaneBot/1.0 (+https://bursa-en-yakin-eczane.vercel.app)",
      Accept: "text/html",
    },
  });
  ...
  const eczaneler = parseNobetciHtml(html);
